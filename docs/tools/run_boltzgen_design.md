...
| `binder_chain_id` | string | no | `C` | pattern: `^[A-Za-z0-9]$` | Chain id given to the designed chain in the output. Must not be one of `target_chains`: two entities claiming one id is a spec BoltzGen accepts and then behaves unpredictably on. This is also the chain that holds the design when a later step asks. |
