...
| `contig` | string | yes | `—` | pattern: `^(?:/\|[A-Za-z]?\d+-\d+)(?:;(?:/\|[A-Za-z]?\d+-\d+))*$` | Protpardelle-1c's own contig grammar (`;/;` for a chain break). See the doc's worked examples. A malformed contig is rejected here rather than failing deep inside the engine. |
