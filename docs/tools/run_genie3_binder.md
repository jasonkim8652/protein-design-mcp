...
- `run_protpardelle` (not yet implemented) is the direct sibling --
  another target-conditioned binder generator, using RFdiffusion-style
  contig strings instead of a target-PDB-plus-hotspots JSON problem, and
  a materially different (non-latent-diffusion) architecture.
- Neither this tool nor `run_protpardelle` scores the result. Refold with
  `run_chai1` (not yet implemented) or `run_boltz` (not yet implemented)
  with the target chain present, then score the interface with
  `run_ipsae`.
