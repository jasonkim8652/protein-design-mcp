...
result yourself afterward with `run_esmfold2` (not yet implemented) +
`run_ipsae` if you need that number.
...
- `run_genie2`, `run_frameflow` and `run_genie3_scaffold`
  (not yet implemented) generate backbones only; this tool additionally
  gives you a designed sequence for free, at the cost of MultiFlow's own
  (currently unscoreable on this host) refold step being skipped.
- `run_la_proteina` (not yet implemented) also produces a full atomic
  model with sequence, via a materially different (latent, autoencoder-
  mediated) architecture rather than joint discrete/continuous
  flow-matching.
- Either sequence this tool returns can still be independently verified:
  fold it with `run_chai1` (not yet implemented) or `run_boltz`
  (not yet implemented) and score the result with `run_ipsae`.
