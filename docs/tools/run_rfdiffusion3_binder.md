...
| `contig` | string | yes | `—` | pattern: `^(?=.*(^\|,)/\d+(,\|$))(?=.*(^\|,)\d+(-\d+)?(,\|$))(?=.*(^\|,)[A-Za-z]\d+(-\d+)?(,\|$))([A-Za-z]\d+(-\d+)?\|/\d+\|\d+(-\d+)?)(,([A-Za-z]\d+(-\d+)?\|/\d+\|\d+(-\d+)?))*$` | RFdiffusion3's own contig grammar (see the doc's "contig and select_hotspots" section) -- comma-separated tokens, NO spaces: a diffused-length range like "50-50", a bare "/0" chain break, or a target segment like "A1-150" pulled from target_pdb. By convention the diffused (binder) segment comes FIRST for a binder design, the opposite order from run_rfdiffusion_binder's contigs. All three parts are REQUIRED here: omit the "/0" and the binder is generated fused to the target as a single chain, which no later tool can undo. |
