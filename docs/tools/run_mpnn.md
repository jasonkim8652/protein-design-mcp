...
| `backbone_pdb` | string | yes | `—` | pattern: `\.(pdb\|cif)(\.gz)?$` | Backbone structure to design a sequence for. A GZIPPED `.pdb.gz` or `.cif.gz` is accepted as-is: `run_rfdiffusion3_binder` writes its `structure_cif` compressed, and ProDy -- which LigandMPNN parses with -- reads it directly, so decompressing first would be work the caller does for nothing. Confirmed in the mpnn environment on RFdiffusion3's own output: 4234 atoms parsed, all of them selected as protein. Most generators return a TWO-CHAIN complex here: target plus design. Name the design with `chains_to_design` or every chain is redesigned, including the target -- which chain that is differs per generator, so read the one you used. |
