"""Adapter for Protenix v1 (``scripts/engines/protenix.py``, env
``protenix`` -- plain PyPI install, not editable).
...
def build_args(manifest: Manifest, params: dict[str, Any]) -> list[str]:
    ...
    job = {
        ...
        "dtype": params["dtype"],
        "need_atom_confidence": params["need_atom_confidence"],
    }
    return [json.dumps(job)]


def parse_output(manifest: Manifest, run: CompletedRun) -> dict[str, Any]:
    """Read the first seed/sample's confidence summary plus the collected
    ...
    if not confidence_paths:
        raise ValueError(
            "run_protenix's declared 'confidence_json' output was not "
            f"collected -- no summary confidence file was found. "
            f"run.outputs was: {run.outputs}"
        )
    if not isinstance(confidence_paths, list):
        confidence_paths = [confidence_paths]

    # Protenix names each sample "..._sample_<i>_summary_confidence...json"
    # under seed_<seed>/predictions/ -- pick the lowest (seed, sample) pair
    # deterministically rather than relying on lexicographic path sort,
    # which would put "seed_101" after "seed_2".
    def _sort_key(path: str) -> tuple[int, int]:
        seed_match = re.search(r"seed_(\d+)", path)
        sample_match = re.search(r"_sample_(\d+)", path)
    ...
    return {
        ...
        "model_pinned": "protenix_base_default_v1.0.0",
    }
